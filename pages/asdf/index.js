...
export default function PraticePage() {
 ...
 const handleApi = async (event) => {
  ...
  if (!user) {
    ...
    const file = new File([blob], `myImage_${timestamp}.png`, { type: "image/png" });
    ...
    try {
      const response = await api.post('http://www.ogleproject.kro.kr:8000/api/practice/upload/', formData, {
  ...
  useEffect(() =>{
    ...
    hiddenContext.fillRect(0, 0, hidden.width, hidden.height);
...
const getCoordinates = (nativeEvent) => {
  const { pageX, pageY } = nativeEvent.touches ? nativeEvent.touches[0] : nativeEvent;
  const canvasRect = canvasRef.current.getBoundingClientRect();  
  return {
    offsetX: pageX - window.scrollX - canvasRect.left,
    offsetY: pageY - window.scrollY - canvasRect.top
  };
};
  ...
  useEffect(() => {
    const Fetchsentence = async () => {
      const result = await api.get('http://www.ogleproject.kro.kr:8000/api/practice/sentence/')
          ...
          <LayoutHeader />
            ...
            <Function>
              ...
              <Mydiv5>
                ...
                <br />
                <Upload
                    action="http://www.ogleproject.kro.kr/"
